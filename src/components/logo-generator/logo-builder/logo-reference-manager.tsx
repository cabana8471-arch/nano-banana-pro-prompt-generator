...
import type { LogoReference, LogoReferenceType, CreateLogoReferenceInput } from "@/lib/types/logo";

interface LogoReferenceManagerProps {
  logoReferences: LogoReference[];
  selectedReferenceIds: string[];
  isLoading: boolean;
  onCreateReference: (input: CreateLogoReferenceInput, image: File) => Promise<LogoReference | null>;
  onDeleteReference: (id: string) => Promise<boolean>;
  onSelectionChange: (ids: string[]) => void;
}

const referenceTypeIcons: Record<LogoReferenceType, React.ReactNode> = {
...
export function LogoReferenceManager({
  ...
  onDeleteReference,
  onSelectionChange,
}: LogoReferenceManagerProps) {
  const t = useTranslations("logoGenerator.referenceImages");
  const tCommon = useTranslations("common");

  const [uploadDialogOpen, setUploadDialogOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [referenceToDelete, setReferenceToDelete] = useState<LogoReference | null>(null);
  ...
  const [referenceType, setReferenceType] = useState<LogoReferenceType>("style");
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const maxReferences = RESOURCE_LIMITS.MAX_LOGO_REFERENCES_PER_GENERATION;
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const input: CreateLogoReferenceInput = {
        name: name.trim(),
        referenceType,
      };
      if (description.trim()) {
  ...
  const openDeleteDialog = (reference: LogoReference, e: React.MouseEvent) => {
    e.stopPropagation();
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={referenceType} onValueChange={(v) => setReferenceType(v as LogoReferenceType)}>
                <SelectTrigger>
